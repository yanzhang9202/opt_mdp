function [xu, J, pi] = solver_mdp_alpha(ref, instance, alpha)
...
for cnt_itr = 1 : maxIter
    ...
    for sx = 0 : 4  % Compute the current state value given the current J
        for sy = 0 : 4
            for st = 1 : maxH
                ...
                for aa = 1 : nAct
                   [Js(aa), prior(aa)] = ...
                       model_value_update_alpha(s, aa, J, instance, ref, alpha); 
                end
    ...
    if Jinov_max < Jeps
        ...
        if mdpverbose
            if mod(cnt_itr, 10) == 1
            fprintf(['MDP ', num2str(cnt_itr), ' th iters:', ...
